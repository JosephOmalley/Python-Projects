...
def create_db(self):
    conn = sqlite3.connect("Stracker.db")
    with conn:
        cur = conn.cursor()
...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_Student (col_fname,col_lname,col_fullname,col_phone,col_email,col_Course) VALUES (?,?,?,?,?,?)""", ("Billy", "Bob", "Billy Bob", "123-456-7891", "bBob@email.com", "Chem"))
            conn.commit()
    conn.close()
