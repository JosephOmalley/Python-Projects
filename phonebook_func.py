...
# So we can habe access to themm
import phonebook_Tkinter
import phonebook_gui


def center_window(self, w, h): # pass in the tkinter frame (master_ reference and the w and h
...
#cath if the user's clicks on the windows upper-right 'X' to ensure they want to close
def ask_quit(self):
...
def first_run(self):
    ...
    with conn:
        ...
        if count < 1:
            cur.execute("""INSERT INTO tbl_phonebook (col_fname,col_lname,col_fullname,col_phone,col_email) VALUES (?,?,?,?,?)""", ("Name", "Example", "Name Example", "123-456-7891", "Nexa@email.com"))
            conn.commit()
    conn.close()
